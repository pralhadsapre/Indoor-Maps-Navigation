...
/*		
Log.w("bestfit", ToolBox.tdp(nav.getNavPathLen() - nav.getNavPathWalked())+" ");
Log.w("bestfit", conf.npPointer + " " + edges.size());
		if(ToolBox.tdp(nav.getNavPathLen() - nav.getNavPathWalked()) < 2.0f) {
			nav.setNavigating(false);
			if(conf.npPointer <= edges.size())
				conf.npPointer++;
Log.w("bestfit", "inside " + conf.npPointer + " " + edges.size());
		} */
