...
//VHSIZE WILL CONTROL HOW MANY PREVIOUS READINGS ARE STORED
...
//LOG COMMAND - MIGHT CONSIDER REMOVING IT LATER ON
			Log.i("inMAPS", "Detected step  in  round = " + round  + " @ "+ now_ms);
		
		}
		round++;
	}

//DATA STORED IN THE ARRAY - THE ACCELEROMETER READING (RIGHT NOW IT HOLDS A TOTAL OF 5 PREVIOUS AND 1 NEW VALUE)
...
//MY CALCULATIONS SHOW ADDING VHSIZE ONLY ONCE IS REQUIRED TO MOVE THE INDEX IN REVERSE ORDER FOR THE FIRST CONDITION IN IF		
		for(int t = 1; t <= lookahead; t++){
			if((values_history[(vhPointer - 1 - t + vhSize + vhSize) % vhSize] - 
					values_history[(vhPointer - 1 + vhSize) % vhSize]
			                   > diff)){
				Log.i("inMAPS", "Detected step with t = " + t + ", diff = " + diff + " < " + (values_history[(vhPointer - 1 - t + vhSize + vhSize) % vhSize] - 
						values_history[(vhPointer - 1 + vhSize) % vhSize]));
				return true;
			}
		}
