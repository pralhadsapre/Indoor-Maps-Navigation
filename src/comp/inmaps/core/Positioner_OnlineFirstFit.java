...
import comp.inmaps.gui.Navigator;
...
//THE INFO. THAT A STEP HAS BEEN DETECTED IS INDICATED BY totalStepsWalked AND dirHistory IS ADDED THE COMPASS BEARING (DIRECTION) 
/*
Log.w("firstfit", ToolBox.tdp(nav.getNavPathLen() - nav.getNavPathWalked())+" ");
Log.w("firstfit", conf.npPointer + " " + navPathEdges.size());
		if(ToolBox.tdp(nav.getNavPathLen() - nav.getNavPathWalked()) < 1.0f) {
			nav.setNavigating(false);
			if(conf.npPointer <= navPathEdges.size())
				conf.npPointer++;
Log.w("firstfit", "inside " + conf.npPointer + " " + navPathEdges.size());
		} */
...
//npStepSize IS THE STEP SIZE WHICH IS CALCULATED FROM THE HEIGHT ENTERED IN CENTIMETERS
				
//				Log.i("inMAPS", "Current edge matches ");
...
//THUS IF STEP IS DETECTED AND THE EDGE IS A STAIR EDGE THEN DISTANCE IS UPDATED DEPENDING ON THE PATHLEN/STEP_COUNT FOR EACH MOVEMENT (STEP DETECTION) 
...
//				Log.i("inMAPS", "Unmatched steps  = " + conf.npUnmatchedSteps);
...
//		Log.i("inMAPS", "Searching for " + lastDir);
//		Log.i("inMAPS", "Current edge " + npC.npPointer);
...
//INCIDENTALLY minCount IS SET TO THE SAME VALUE AS maxFallBackSteps
...
//			Log.i("inMAPS", "Found edge (" + localPointer + ") in direction " + navPathEdges.get(localPointer).getCompDir());
...
//				Log.i("inMAPS", "Edge (" + localPointer + ") is in range");
...
//				Log.i("inMAPS", "Summing up path length " + lastDir);
//				Log.i("inMAPS", "backlogPointer = " + backLogPointer + ">" + npC.npPointer + " npLastMatchedStep");
				while(backLogPointer > npC.npPointer && findMatchingSteps(o, edgeDir, npC, edgeLength, npC.npStepSize)){
					oldCount = o.count;
//					Log.i("inMAPS", "Found matching steps into edgeDir = " + edgeDir + ", oldCount = " 
...
//				Log.i("inMAPS", "Found " + oldCount + " matching steps");
...
//				Log.i("inMAPS", "Edge (" + localPointer + ") not in range");
			}
		}
		
		if(maxBackLogCount != Integer.MIN_VALUE){
//			Log.i("inMAPS", "Found new position with " + (double)maxBackLogCount/(double)(dirHistory.size()-1-npC.npLastMatchedStep) + " confidence");
	...
	private boolean findMatchingSteps(UglyObject o, double edgeDir, NPConfig npC, double edgeLength, double stepLength){
		int oldCount = o.count;
//		Log.i("inMAPS", "findMatchingSteps: " + oldCount);
...
//				Log.i("inMAPS", "findMatchingSteps/ edgeLength (" + edgeLength + ") <= "
//						+ (o.count-oldCount)*lengthToAdd);
				// return true if whole edge has been traveled along
				return true;
			}
		}
//		Log.i("inMAPS", "found: " + oldCount);
		if(oldCount != o.count){
			return true;
		} else {
			return false;
...
//			Log.i("inMAPS", "Adding amount into direction " + edge.getCompDir());
			historyPointer--;
			double lengthToAdd = 0.0;
			if(edge.isStairs()){
				// Don't use step length because of stairs
